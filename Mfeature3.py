...
# 读取测试文件
voice_test = pd.read_csv('JDATA_TEST/voice_test_a.txt', sep='\t',header=None, names=('uid','opp_num','opp_head','opp_len','start_time','end_time','call_type','in_out'),dtype={'start_time':str,'end_time':str})
sms_test = pd.read_csv('JDATA_TEST/sms_test_a.txt', sep='\t',header=None, names=('uid','opp_num','opp_head','opp_len','start_time','in_out'),dtype={'start_time':str})
wa_test = pd.read_csv('JDATA_TEST/wa_test_a.txt', sep='\t',header=None, names=('uid','wa_name','visit_cnt','visit_dura','up_flow','down_flow','wa_type','date'),dtype={'date':str})

# 数据整理
uid_test = pd.DataFrame({'uid': pd.unique(wa_test['uid'])})
uid_test.to_csv('JDATA_TEST/uid_test_a.txt', index=None)
...
voice_date_x = voice.start_time.apply(day_count)
voice['day_count'] = voice_date_x
voice_date = voice.groupby(['uid', 'day_count'])['uid'].count().unstack().add_prefix('voice_day_').reset_index().fillna(0)
...
voice_feature = pd.merge(voice_feature, voice_date, how='left', on='uid')
...
sms_date_x = sms.start_time.apply(day_count)
sms['day_count'] = sms_date_x
sms_date = sms.groupby(['uid', 'day_count'])['uid'].count().unstack().add_prefix('sms_day_').reset_index().fillna(0)
...
sms_feature = pd.merge(sms_feature, sms_date, how='left', on='uid')
...
# 网站和APP的统计
web_app = wa.groupby(['uid', 'wa_type'])['uid'].count().unstack().add_prefix('woa_').reset_index().fillna(0)


# 每日网站访问统计
w_date = wa.groupby(['uid', 'date'])['uid'].count().unstack().add_prefix('date_').reset_index().fillna(0)
...
wa_feature = pd.merge(wa_feature, web_app, how='left', on='uid')
wa_feature = pd.merge(wa_feature, w_date, how='left', on='uid')
...
train_feature.to_csv('MY/train_featureV1.csv',index=None)
test_feature.to_csv('MY/test_featureV1.csv',index=None)
